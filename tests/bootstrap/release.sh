rm -rf tests/bootstrap/tmp/release
...
./build/tools/asc/warpo_asc assemblyscript/src/glue/wasm/index.ts assemblyscript/src/index-wasm.ts \
  ...
  --initialMemory 768 \
  --exportStart _initialize --exportRuntime --exportTable \
  --asc-wasm tests/bootstrap/tmp/release/assemblyscript.warpo-test.wasm \
  --use ASC_GC_TESTING=1 ASC_GC_IDLEFACTOR=0 ASC_GC_GRANULARITY=64 ASC_GC_SWEEPFACTOR=1000 \
...
./build/tools/asc/warpo_asc assemblyscript/src/glue/wasm/index.ts assemblyscript/src/index-wasm.ts \
  ...
  --initialMemory 768 \
  --exportStart _initialize --exportRuntime --exportTable \
  --asc-wasm tests/bootstrap/tmp/release/assemblyscript.warpo-test-bootstrap.wasm \
  --use ASC_GC_TESTING=1 ASC_GC_IDLEFACTOR=0 ASC_GC_GRANULARITY=64 ASC_GC_SWEEPFACTOR=1000 \
  -o tests/bootstrap/tmp/release/assemblyscript.warpo-test-bootstrap.wasm
  
git --no-pager diff --no-index tests/bootstrap/tmp/release/assemblyscript.warpo-test.wat tests/bootstrap/tmp/release/assemblyscript.warpo-test-bootstrap.wat
